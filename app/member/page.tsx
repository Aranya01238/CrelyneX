...
export default function MemberPage() {
  ...
  const handleMarkFinished = async (id: string) => {
    try {
      ...
      if (res.ok) {
        setTasks((prev) =>
          prev.map((t) => (t.id === id ? { ...t, memberMarkedDone: true } : t)),
        );
        ...
        <div className="lg:col-span-12 xl:col-span-8 flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="space-y-3">
            ...
            <p className="text-zinc-500 text-sm sm:text-base font-medium max-w-xl italic leading-relaxed">
              Your centralized node for accessing CrelyneX resources, tracking
              mission objectives, and managing digital growth.
            </p>
          </div>
          <div className="flex items-center gap-6 glass p-4 rounded-[32px] border-white/5 h-fit">
            <div className="text-right">
              <div className="text-[10px] font-black text-purple-400 uppercase tracking-[0.3em] mb-1">
                Mission Credits
              </div>
        ...
        <div className="lg:col-span-8 space-y-10 reveal-left delay-100">
          <div className="space-y-6">
            <h3 className="text-xl font-bold text-white flex items-center gap-3">
              <ShieldCheck className="w-5 h-5 text-purple-500" /> Authorized
              Nodes
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {portalCards
                .filter((p) => portals.includes(p.id))
                .map((portal) => (
                  <Link
                    key={portal.id}
                    href={portal.href}
                  ...
                  >
                    ...
                    <Card className="h-full border-white/5 bg-[#0a0a0a]/80 backdrop-blur-xl rounded-[40px] overflow-hidden shadow-2xl relative z-10">
                      <div
                        className={`absolute inset-0 bg-gradient-to-br ${portal.color} opacity-40`}
                      />
              ...
              {portals.length === 0 && (
                <div className="col-span-full py-24 glass rounded-[40px] border-dashed border-white/10 text-center space-y-4">
                  ...
                  <p className="text-zinc-500 text-sm font-black uppercase tracking-widest leading-relaxed">
                    No Authorized Portal Connections Found.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-4">
              {isLoading ? (
                ...
                tasks.map((task) => (
                  <div
                    key={task.id}
                    className="glass p-6 rounded-[32px] border-white/5 space-y-4 transition-all hover:bg-white/5 relative overflow-hidden group/task"
                  >
                    <div className="flex items-start justify-between relative z-10">
                      <div className="space-y-1">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-[9px] font-black bg-purple-500/10 text-purple-400 px-1.5 py-0.5 rounded border border-purple-500/10">
                            {task.points} CP
                          </span>
                        </div>
                    ...
                    <p className="text-xs text-zinc-500 italic leading-relaxed wrap-break-word whitespace-pre-wrap relative z-10">
                      "{task.description}"
                    </p>
