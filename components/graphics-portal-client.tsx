...
import { Loader2, Image as ImageIcon, Video, Trash2, Plus, UploadCloud, PlayCircle, ExternalLink } from "lucide-react";
            ...
            <form onSubmit={handleAdd} className="space-y-6">
              <div className="space-y-4">
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-600">Classification</label>
                  <Select 
                    value={form.type} 
                  ...
                  >
                    ...
                    <SelectContent className="bg-[#0a0a0a] border-white/10">
                      <SelectItem value="poster">Static Poster (Image)</SelectItem>
                      <SelectItem value="video">Dynamic Video (URL)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-600">Asset Label</label>
                  <Input 
                    value={form.title}
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-600">Digital Location (URL)</label>
                  <Input 
                    value={form.url}
        ...
        <div className="lg:col-span-8 flex flex-col items-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full reveal-right delay-300">
                {uploads.map((upload, i) => (
                    <Card key={upload.id} className="group overflow-hidden border-white/5 bg-[#0a0a0a] rounded-[32px] hover:bg-white/5 transition-all shadow-xl">
                        <div className="relative aspect-video glass overflow-hidden flex items-center justify-center p-2">
                            ...
                            <div className="absolute top-4 right-4 z-10">
                                <a href={upload.url} target="_blank" rel="noopener noreferrer" className="h-10 w-10 glass rounded-full flex items-center justify-center text-zinc-400 hover:text-white transition-all">
                        ...
                        <CardContent className="p-6 relative">
                            <div className="flex items-start justify-between gap-4">
                                <div className="space-y-1">
                                    <h4 className="text-sm font-black text-white uppercase tracking-wide truncate">{upload.title}</h4>
                                    <div className="flex items-center gap-2 text-[8px] font-bold tracking-[0.2em] text-zinc-500 uppercase">
                                        <div className="flex items-center gap-1">
                                            {upload.type === "poster" ? <ImageIcon className="w-2.5 h-2.5" /> : <Video className="w-2.5 h-2.5" />}
                                            {upload.type}
                                        </div>
                ...
                {uploads.length === 0 && !isLoading && (
                    <div className="col-span-2 py-32 glass rounded-[40px] border-dashed border-white/10 text-center flex flex-col items-center justify-center gap-4">
                        ...
                        <p className="text-zinc-600 text-xs font-bold uppercase tracking-widest italic">Digital archive is currently empty.</p>
                    </div>
                )}
            </div>
        </div>
